...
// Search from extension with no point.
const char * MimeTypes::searchType(const char *extension){
  int min = 0;
...
// Assumed no point before extension.
const char* MimeTypes::getType(const char * name)  {
  const char* extension = strrchr(name, '.'); // Returns pointer to . if found.
  return searchType(extension ? extension++ : name); 
}
