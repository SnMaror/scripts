...
UtilError::UtilError(int error_code, const char* fmt, ...) noexcept {
    va_list args;
    va_start(args, fmt);
    ...
    if (s < 0) {
         ...
         *
         * Lanzar una excepción no es una opción: `UtilError` es una excepción
...
const char* UtilError::what() const noexcept { return msg_error; }
